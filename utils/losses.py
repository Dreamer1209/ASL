...
class SCELoss(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        label_one_hot = label_one_hot.permute(0,4,1,2,3)
...
class BaCELoss(torch.nn.Module):
    def __init__(self, alpha, beta, num_classes=10, reduction = "mean"):
        super(BaCELoss, self).__init__()
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.alpha = alpha
        self.beta = beta
        self.num_classes = num_classes
    ...
    def forward(self, pred, labels):
        #CCE
        ce1 = self.cross_entropy(pred, labels).mean()
        ce2 = self.cross_entropy(labels,pred).mean()

        # Loss
        loss = self.alpha * ce2 + self.beta * ce1
        return loss
