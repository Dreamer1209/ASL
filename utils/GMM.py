...
def calculate_matmul_n_times(n_components, mat_a, mat_b):
    ...
    # print("ma",mat_a.shape)
    # print("mb",mat_b.shape)
    # print("cpom",n_components)
    for i in range(n_components):
        mat_a_i = mat_a[:, i, :, :].squeeze(-2)
        mat_b_i = mat_b[0, i, :, :].squeeze()
        # print(mat_b_i)
        # print("m1",mat_b_i.shape)
        # print("m2",mat_a_i.shape)
...
class GaussianMixture(torch.nn.Module):
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "full":
            ...
            log_2pi = torch.tensor(log_2pi).cuda()

            log_det = self._calculate_log_det(precision).cuda()

            x_mu_T = (x - mu).unsqueeze(-2)
            x_mu = (x - mu).unsqueeze(-1)

            x_mu_T_precision = calculate_matmul_n_times(self.n_components, x_mu_T, precision)
            x_mu_T_precision_x_mu = calculate_matmul(x_mu_T_precision, x_mu).cuda()
